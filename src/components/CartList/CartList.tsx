...
const CartList = ({ cartData, reloadCallback } : { cartData : CartProduct[], reloadCallback : Function }) => {
    const [ internalCartData, setInternalCartData ] = useState<CartProduct[]>([]);

    useEffect(() => {
        if (!cartData) {
          const cart = cartService.load();
          if (cart) {
            setInternalCartData(cart);
          }
        } else {
          setInternalCartData(cartData);
        }
      }, [cartData]);
  ...
  const handleRemoveItem = (item : CartProduct) => {
    ...
    setInternalCartData([...updatedCart]);
    reloadCallback();
    ...
    <section className="w-100 pt-5 row">
        <div className="col-xl-9 col-lg-8 col-12 px-3">
            <div className={cx("main-cart")}>
                <h1>Giỏ hàng của bạn <b className="price-txt">{ internalCartData.length } Sản Phẩm</b></h1>
        ...
        <div className="col-xl-3 col-lg-4 col-12 px-3">
            <CartSummary cartData={internalCartData}/>
        </div>
    </section>
  );
};
